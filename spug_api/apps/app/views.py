...
class DeployView(View):
    ...
    def get(self, request):
        form, error = JsonParser(Argument("app_id", type=int, required=False)).parse(
            request.GET, True
        )
        if not request.user.is_supper:
    ...
    def post(self, request):
        form, error = JsonParser(
            ...
            Argument(
                "extend", filter=lambda x: x in dict(Deploy.EXTENDS), help="请选择发布类型"
            ),
        ...
        if error is None:
            deploy = Deploy.objects.filter(
                app_id=form.app_id, env_id=form.env_id
            ).first()
            ...
            if form.extend == "1":
                extend_form, error = JsonParser(
                    ...
                    Argument(
                        "versions", type=int, filter=lambda x: x > 0, help="请输入发布保留版本数量"
                    ),
                ...
                extend_form.require_upload = any(
                    x.get("src_mode") == "1" for x in extend_form.host_actions
                )
