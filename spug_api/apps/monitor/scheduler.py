...
class Scheduler:
    ...
    def __init__(self):
        self.scheduler = BackgroundScheduler(
            timezone=self.timezone, executors={"default": ThreadPoolExecutor(30)}
        )
    ...
    def _handle_event(self, event):
        ...
        if event.code == EVENT_SCHEDULER_SHUTDOWN:
            ...
            Notify.make_notify(
                "monitor", "1", "调度器已关闭", "调度器意外关闭，你可以在github上提交issue", False
            )
            ...
            Notify.make_notify(
                "monitor", "1", f"{obj.name} - 执行异常", f"{event.exception}"
            )
    ...
    def _dispatch(self, task_id, tp, targets, extra, threshold, quiet):
        ...
        for t in json.loads(targets):
            rds_cli.rpush(
                MONITOR_WORKER_KEY,
                json.dumps([task_id, tp, t, extra, threshold, quiet]),
            )
        ...
        while True:
            ...
            if task.action in ("add", "modify"):
                trigger = IntervalTrigger(
                    minutes=int(task.rate), timezone=self.timezone
                )
                self.scheduler.add_job(
                    self._dispatch,
                    trigger,
