...
class ConfigView(View):
    ...
    def post(self, request):
        ...
        if error is None:
            ...
            envs = form.pop("envs")
            for env_id in envs:
                cf = Config.objects.filter(
                    o_id=form.o_id, type=form.type, env_id=env_id, key=form.key
                ).first()
                if cf:
        ...
        if error is None:
            ...
            if config.value != form.value:
                ...
                ConfigHistory.objects.create(
                    action="2", old_value=old_value, **config.to_dict(excludes=("id",))
                )
            config.save()
        return json_response(error=error)
    ...
    @auth("config.src.edit_config|config.app.edit_config")
    def delete(self, request):
        form, error = JsonParser(Argument("id", type=int, help="未指定操作对象")).parse(
            request.GET
        )
        if error is None:
            config = Config.objects.filter(pk=form.id).first()
            if config:
                ConfigHistory.objects.create(
                    ...
                    **config.to_dict(
                        excludes=("id", "value", "updated_at", "updated_by_id")
                    ),
                )
                config.delete()
        return json_response(error=error)
    ...
    for item in Config.objects.filter(**query):
        if item.key in data:
            ...
            if item.value != value:
                ...
                item.save()
                ConfigHistory.objects.create(
                    action="2", old_value=old_value, **item.to_dict(excludes=("id",))
                )
        else:
            ConfigHistory.objects.create(
                action="3",
