...
def compose_configs(app, env_id, no_prefix=False):
    configs = dict()
    # app own configs
    for item in Config.objects.filter(type="app", o_id=app.id, env_id=env_id).only(
        "key", "value"
    ):
    ...
    if app.rel_apps:
        ...
        if app_ids:
            ...
            for item in Config.objects.filter(
                type="app", o_id__in=app_ids, env_id=env_id, is_public=True
            ).only("key", "value"):
    ...
    if app.rel_services:
        ...
        if src_ids:
            ...
            for item in Config.objects.filter(
                type="src", o_id__in=src_ids, env_id=env_id
            ).only("key", "value"):
                key = item.key if no_prefix else f"{id_key_map[item.o_id]}_{item.key}"
                configs[key] = item.value
    return configs
