...
class FileView(View):
    ...
    def get(self, request):
        form, error = JsonParser(
            Argument("id", type=int, help="参数错误"), Argument("path", help="参数错误")
        ).parse(request.GET)
        if error is None:
...
class ObjectView(View):
    ...
    def get(self, request):
        form, error = JsonParser(
            Argument("id", type=int, help="参数错误"), Argument("file", help="请输入文件路径")
        ).parse(request.GET)
        if error is None:
            if not has_host_perm(request.user, form.id):
                return json_response(error="无权访问主机，请联系管理员")
            ...
            return FileResponseAfter(
                ssh_cli.close, f, as_attachment=True, filename=filename
            )
        return json_response(error=error)
