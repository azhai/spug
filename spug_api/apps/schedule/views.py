...
class Schedule(View):
    ...
    def post(self, request):
        form, error = JsonParser(
            ...
            Argument(
                "trigger", filter=lambda x: x in dict(Task.TRIGGERS), help="请选择触发器类型"
            ),
        ...
        if error is None:
            ...
            if form.get("is_active") is not None:
                ...
                if form.is_active:
                    ...
                    message.update(
                        task.to_dict(
                            selects=("trigger", "trigger_args", "command", "targets")
                        )
                    )
                else:
    ...
    if error is None:
        ...
        if run_time:
            return json_response(
                {"success": True, "msg": run_time.strftime("%Y-%m-%d %H:%M:%S")}
            )
        else:
