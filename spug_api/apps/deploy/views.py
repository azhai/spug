...
class RequestDetailView(View):
    ...
    def get(self, request, r_id):
        ...
        if req.is_quick_deploy:
            if outputs["local"]["data"]:
                outputs["local"]["data"] = (
                    f"{human_time()} 读取数据...        " + outputs["local"]["data"]
                )
            else:
    ...
    def post(self, request, r_id):
        ...
        outputs = {
            x.id: {"id": x.id, "title": x.name, "step": 0, "data": message}
            for x in hosts
        }
        req.status = "2"
        ...
        if req.deploy.extend == "2":
            ...
            if req.deploy.extend == "2":
                ...
                return json_response(
                    {"s_actions": s_actions, "h_actions": h_actions, "outputs": outputs}
                )
...
def post_request_ext1_rollback(request):
    ...
    if error is None:
        ...
        requests = DeployRequest.objects.filter(
            deploy=req.deploy, status__in=("3", "-3")
        )
        ...
        if req.deploy.is_audit:
            Thread(
                target=Helper.send_deploy_notify, args=(new_req, "approve_req")
            ).start()
    return json_response(error=error)
...
def post_request_ext2(request):
    ...
    if error is None:
        ...
        if DeployExtend2.objects.filter(
            deploy=deploy, host_actions__contains='"src_mode": "1"'
        ).exists():
            if not extra:
