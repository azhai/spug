    ...
    for action in host_actions:
        if action.get("type") == "transfer":
            if action.get("src_mode") == "1":
                break
            helper.send_info(
                "local", f"{human_time()} 检测到来源为本地路径的数据传输动作，执行打包...   \r\n"
            )
            ...
            if not action["src"] or not action["dst"]:
                helper.send_error(
                    "local",
                    f'invalid path for transfer, src: {action["src"]} dst: {action["dst"]}',
                )
    ...
    if host_actions:
        if req.deploy.is_parallel:
            ...
            while host_ids:
                ...
                try:
                    _deploy_ext2_host(
                        helper, h_id, host_actions, new_env, req.spug_version
                    )
                except Exception as e:
                    helper.send_error(h_id, f"Exception: {e}", False)
                    for h_id in host_ids:
...
def _deploy_ext1_host(req, helper, h_id, env):
    ...
    with host.get_ssh(default_env=env) as ssh:
        ...
        helper.remote_raw(
            host.id,
            ssh,
            f"rm -f {extend.dst_dir} && ln -sfn {repo_dir} {extend.dst_dir}",
        )
...
def _deploy_ext2_host(helper, h_id, actions, env, spug_version):
    ...
    with host.get_ssh(default_env=env) as ssh:
        for index, action in enumerate(actions):
            helper.send_step(
                h_id, 1 + index, f'{human_time()} {action["title"]}...\r\n'
            )
            if action.get("type") == "transfer":
                if action.get("src_mode") == "1":
                    try:
                        ssh.put_file(
                            os.path.join(REPOS_DIR, env.SPUG_DEPLOY_ID, spug_version),
                            action["dst"],
                        )
                    except Exception as e:
                        helper.send_error(host.id, f"Exception: {e}")
                    ...
                    try:
                        ssh.put_file(
                            os.path.join(sp_dir, tar_gz_file), f"/tmp/{tar_gz_file}"
                        )
                    except Exception as e:
                        helper.send_error(host.id, f"Exception: {e}")
                    ...
                    command += (
                        f'&& rm -rf /tmp/{spug_version}* && echo "transfer completed"'
                    )
            else:
