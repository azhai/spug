...
def fetch_ali_instances(ak, ac, region_id, page_number=1):
    ...
    for item in res["Instances"]["Instance"]:
        ...
        data[item["InstanceId"]] = dict(
            ...
            instance_charge_type=check_instance_charge_type(
                item["InstanceChargeType"], "ali"
            ),
            internet_charge_type=check_internet_charge_type(
                item["InternetChargeType"], "ali"
            ),
            public_ip_address=item["PublicIpAddress"]["IpAddress"],
            private_ip_address=list(
                map(lambda x: x["PrimaryIpAddress"], network_interface)
            ),
            zone_id=item["ZoneId"],
        )
    ...
    for item in res["Response"]["InstanceSet"]:
        ...
        data.append(
            dict(
                ...
                expired_time=parse_utc_date(item["ExpiredTime"]),
                instance_charge_type=check_instance_charge_type(
                    item["InstanceChargeType"], "tencent"
                ),
                internet_charge_type=check_internet_charge_type(
                    internet_charge_type, "tencent"
                ),
...
def fetch_host_extend(ssh):
    ...
    with ssh:
        ...
        code, out = ssh.exec_command_raw(
            "dmidecode -t 17 | grep -E 'Size: [0-9]+' | awk '{s+=$2} END {print s,$3}'"
        )
        if code == 0:
...
def batch_sync_host(token, hosts, password):
    ...
    with futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        for host in hosts:
            t = executor.submit(
                _sync_host_extend, host, private_key, public_key, password
            )
            t.host = host
            threads.append(t)
        for t in futures.as_completed(threads):
            ...
            if exception:
                rds.rpush(
                    token,
                    json.dumps(
                        {"key": t.host.id, "status": "fail", "message": f"{exception}"}
                    ),
                )
            else:
