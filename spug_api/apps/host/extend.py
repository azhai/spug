...
class ExtendView(View):
    ...
    def get(self, request):
        form, error = JsonParser(Argument("host_id", type=int, help="参数错误")).parse(
            request.GET
        )
        if error is None:
    ...
    def post(self, request):
        form, error = JsonParser(
            ...
            Argument(
                "private_ip_address", type=list, filter=lambda x: len(x), help="请添加内网IP"
            ),
        ...
        if error is None:
            ...
            form.public_ip_address = (
                json.dumps(form.public_ip_address) if form.public_ip_address else "[]"
            )
