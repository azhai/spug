        ...
        if error is None:
            ...
            try:
                if form.pkey:
                    ...
                    with SSH(
                        form.hostname, form.port, form.username, password=password
                    ) as ssh:
            ...
            if form.id:
                ...
                host = Host.objects.create(
                    created_by=request.user, is_verified=True, **form
                )
    ...
    def delete(self, request):
        ...
        if error is None:
            ...
            for host_id in host_ids:
                ...
                deploy = (
                    Deploy.objects.filter(host_ids__regex=regex)
                    .annotate(app_name=F("app__name"), env_name=F("env__name"))
                    .first()
                )
                if deploy:
                ...
                task = Task.objects.filter(
                    targets__regex=fr"[^0-9]{form.id}[^0-9]"
                ).first()
                if task:
                    return json_response(
                ...
                detection = Detection.objects.filter(
                    type__in=("3", "4"), targets__regex=regex
                ).first()
                if detection:
                    return json_response(
...
def post_import(request):
    ...
    for i, row in enumerate(ws.rows):
        ...
        if Host.objects.filter(
            hostname=data.hostname, port=data.port, username=data.username
        ).exists():
...
def batch_valid(request):
    ...
    if error is None:
        ...
        return json_response(
            {"token": token, "hosts": {x.id: {"name": x.name} for x in hosts}}
        )
    return json_response(error=error)
