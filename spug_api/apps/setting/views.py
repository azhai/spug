...
class SettingView(AdminView):
    ...
    def post(self, request):
        form, error = JsonParser(Argument("data", type=list, help="缺少必要的参数")).parse(
            request.body
        )
        if error is None:
...
class MFAView(AdminView):
    ...
    def post(self, request):
        form, error = JsonParser(
            Argument("enable", type=bool, help="参数错误"), Argument("code", required=False)
        ).parse(request.body)
        if error is None:
            if form.enable:
                if not form.code:
...
def ldap_test(request):
    ...
    if error is None:
        try:
            con = ldap.initialize(
                "ldap://{0}:{1}".format(form.server, form.port), bytes_mode=False
            )
