...
def send_login_wx_code(wx_token, code):
    ...
    res = requests.post(
        url, json={"token": spug_key, "user": wx_token, "code": code}, timeout=30
    )
...
class Notification:
    ...
    def _by_wx(self):
        if not self.spug_key:
            Notify.make_notify(
                notify_source, "1", "发送报警信息失败", "未配置报警服务调用凭据，请在系统管理/系统设置/基本设置/调用凭据中配置。"
            )
            return
        users = set(
            x.wx_token
            for x in Contact.objects.filter(id__in=self.u_ids, wx_token__isnull=False)
        )
        if users:
            data = {
                "token": self.spug_key,
            ...
            Notify.make_notify(
                notify_source, "1", "发送报警信息失败", "未找到可用的通知对象，请确保设置了相关报警联系人的微信Token。"
            )

    def _by_email(self):
        users = set(
            x.email
            for x in Contact.objects.filter(id__in=self.u_ids, email__isnull=False)
        )
        if users:
            ...
            if mail_service.get("server"):
                ...
                Notify.make_notify(
                    notify_source, "1", "发送报警信息失败", "未配置报警服务调用凭据，请在系统管理/系统设置/报警服务设置中配置。"
                )
        else:
            Notify.make_notify(
                notify_source, "1", "发送报警信息失败", "未找到可用的通知对象，请确保设置了相关报警联系人的邮件地址。"
            )

    def _by_dd(self):
        users = set(
            x.ding
            for x in Contact.objects.filter(id__in=self.u_ids, ding__isnull=False)
        )
        if users:
            texts = [
            ...
            Notify.make_notify(
                notify_source, "1", "发送报警信息失败", "未找到可用的通知对象，请确保设置了相关报警联系人的钉钉。"
            )

    def _by_qy_wx(self):
        users = set(
            x.qy_wx
            for x in Contact.objects.filter(id__in=self.u_ids, qy_wx__isnull=False)
        )
        if users:
            color, title = (
                ("warning", "监控告警通知") if self.event == "1" else ("info", "告警恢复通知")
            )
            texts = [
                f"## {title}",
            ...
            Notify.make_notify(
                notify_source, "1", "发送报警信息失败", "未找到可用的通知对象，请确保设置了相关报警联系人的企业微信。"
            )
